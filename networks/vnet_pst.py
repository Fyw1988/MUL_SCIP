...
class AttnBlock(nn.Module):
    ...
    def forward_part1(self, x):
        ...
        x_attn = self.attn(x, shape=(W, H, D))  # B*nW, Wd*Wh*Ww, C
        # reshape-->(B, D, H, W, C)
...
class SelfAttention3D(nn.Module):
    ...
    def forward(self, x, shape, frame_len=10):
        ...
        if self.shift:
            ...
            # (B, n_h, D*H*W, C//n_h)
            x = self.shift_op(x, frame_len, shape)   # (B, D, n_h, H, W, c)
            # 按深度为10分层，取第一层算self-attn
            x_slice = x[:, 0:10, :, :, :, :].permute(0, 1, 3, 4, 2, 5).reshape(B, n, C)
            # x = x.permute(0, 2, 1, 3).reshape(B, N, C)
        qk = self.qk(x_slice).reshape(B, n, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)  # (3,B,nH,n,C // self.num_heads)
        # v = self.v(x).view(B, self.num_heads, N, C // self.num_heads)   # (B, n_h, N, c)
        q, k = qk[0], qk[1]  # B, nH, n, c
        ...
        frames = []
        # 现在需要将(B, nH, n, n)的权重用在每一层上
        for i in range(D // 10):
            x_slice = x[:, i*10:(i+1)*10, :, :, :, :].permute(0, 1, 3, 4, 2, 5).reshape(B, n, C)
            # import pdb; pdb.set_trace()
            v_slice = self.v(x_slice).view(B, self.num_heads, n, C // self.num_heads)   # (B, nH, n, c)
            frame = attn @ v_slice   # (B, nH, n, c)
            frames.append(frame)
        x = torch.cat(frames, dim=2)   # (B, nH, N, c)

        if self.shift and self.shift_type == 'psm':
            x = self.shift_op_back(x, frame_len, shape)   # (B, D, nH, H, W, c)
            x = x.transpose(1, 2).reshape(B, self.num_heads, N, -1).view(B, N, C)
        else:
...
class PatchShift(nn.Module):
    ...
    def shift(x, shape, inv=False, ratio=0.5, frame_len=10):
        ...
        out = out.view(B, D, num_heads, H, W, c)
        return out
...
class TemporalShift(nn.Module):
    ...
    def shift(x, shape, fold_div=8):
        ...
        out = feat.clone()
        # 将体从D维度上分为D//10层，每层混有所有层的通道信息
        for i in range(D // 10):
            out[:, 0:10, :, :, fold*i:fold*(i+1)] = feat[:, i*10:(i+1)*10, :, :, fold*i:fold*(i+1)]  # shift
