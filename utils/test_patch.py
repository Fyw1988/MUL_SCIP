...
def var_all_case(model, lgf, num_classes, patch_size, stride_xy, stride_z, dataset_name):
    if dataset_name == "Pancreas":
        with open('/root/data/fyw/Semi-seg./datasets/Pancreas-CT/test.list', 'r') as f:
            image_list = f.readlines()
        image_list = ["/root/data/fyw/Semi-seg./datasets/Pancreas-CT/Pancreas_h5/" + item.replace('\n', '') + "_norm.h5" for item in image_list]
    elif dataset_name == "LA":
        # image_list = glob("/root/data/fyw/Semi-seg./datasets/LA/Testing Set/*/mri_norm2.h5")
        image_list = glob("/root/data/fyw/GF_MT/code/datasets/LA/Testing Set/*/mri_norm2.h5")
    elif dataset_name == "Prostate":
        image_list = glob("./datasets/Prostate/Testing Set/*/mri_norm2.h5")
    elif dataset_name == "BraTS":
        with open('/root/data/fyw/Semi-seg./datasets/BraTS/val.txt', 'r') as f:
            image_list = f.readlines()
        image_list = ["/root/data/fyw/Semi-seg./datasets/BraTS/data/" + item.replace('\n', '') + ".h5" for item in image_list]
    ...
    for image_path in loader:   # 逐张读入val的图像
        ...
        image = h5f['image'][:]
        label = h5f['label'][:]
        prediction, score_map = test_single_case_first_output(model, lgf, image, stride_xy, stride_z, patch_size, num_classes=num_classes)
...
def test_all_case(model_name, num_outputs, model, image_list, num_classes, patch_size, stride_xy, stride_z, lgf, save_result=False, test_save_path=None, preproc_fn=None, metric_detail=1, nms=0):
    ...
    for image_path in loader:
        ...
        prediction, score_map = test_single_case_first_output(model, lgf, image, stride_xy, stride_z, patch_size, num_classes=num_classes)  # (168,146,125), (2,168,146,125)
        if num_outputs > 1:
            prediction_average, score_map_average = test_single_case_average_output(model, lgf, image, stride_xy, stride_z, patch_size, num_classes=num_classes)
        if nms:
...
def test_single_case_first_output(model, lgf, image, stride_xy, stride_z, patch_size, num_classes=1):
